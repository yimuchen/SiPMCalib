...
static TFitResult* RunFit( TH1D& hist, TF1& func,
...
main( int argc, char* argv[] )
{
  ...
  plotdesc.add_options()
    ( "plotlabel", usr::po::defvalue<std::string>( "" ),
...
RunFit( TH1D& hist, TF1& func, const usr::ArgumentExtender& args  )
{
  ...
  const double fitmin = fmin > 0 ? fmin :
                        ftype == "expo" ? x0 + 3*binwidth :
                        x0 - 6*binwidth;
  const double fitmax = fmax > 0 ? fmax :
                        ftype == "expo" ? x0 + ( nbins/2 )*binwidth :
                        x0 + 6*binwidth;
          ...
          const usr::ArgumentExtender& args )
{
  // Additional parsing
  const std::string ftype    = args.Arg<std::string>( "fittype" );
  const std::string fitlabel = ftype == "expo" ? "Exponential Fit" :
                               ftype == "gaus" ?  "Gaussian Fit" :
                               "Custom fit";
  ...
  if( ftype == "expo" ){
    ...
    const double err = fabs( tau * func.GetParError( 1 ) / func.GetParameter( 1 ) );
    if( tau < 1 ){
