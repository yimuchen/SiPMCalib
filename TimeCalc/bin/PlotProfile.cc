...
std::vector<ProfileMgr> MakeProfileList(
...
PairList MakePairs(
  const usr::ArgumentExtender& args, const std::string& arg_name );
...
int main( int argc, char* argv[] )
{
  ...
  desc.add_options()
    ...
    "Input data .txt file" )
    ( "outfile", usr::po::defvalue<std::string>( "profile1D.pdf" ),
    "Prefix of output file name" )
    ( "biasrange", usr::po::multivalue<double>(),
    "Selection range on the bias voltage" )
    ( "tempsipm", usr::po::multivalue<double>(),
    "Selection range on the sipm temperature" )
    ( "temppulser", usr::po::multivalue<double>(),
    "Section ranges on the pulser temperature" )
    ( "xmin", usr::po::defvalue<double>( 25 ),
    "Selection range on the x axis" )
    ( "xmax", usr::po::defvalue<double>( 35 ),
    "Selection range on the x axis" )
    ( "xbins", usr::po::defvalue<int>( 20 ),
  ...
  if( bias_pairs.size() == 1 ){
    const double mean
      = ( bias_pairs.front().first + bias_pairs.front().second ) /2000;
    const double err
      = ( bias_pairs.front().second - bias_pairs.front().first ) /2000;
    c.WriteLine( usr::fstr( "Bias:%.3lf#pm%.3lfV", mean, err ) );
  }
  if( sipm_pairs.size() == 1 ){
    const double mean
      = ( sipm_pairs.front().first + sipm_pairs.front().second ) /2;
    const double err
      = ( sipm_pairs.front().second - sipm_pairs.front().first ) /2;
...
void ProfileMgr::Fill( const double readout,
                       const double bias,
                       ...
                       const double sipmtemp )
{
  if( _bias_min == _bias_max && _bias_min == 0 ){
    ...
    if( _bias_min <= bias  && bias <= _bias_max &&
        _pulse_min <= ledtemp  && ledtemp <= _pulse_max ){
