...
SiPMLowLightFit::PrintTable( std::ostream& sout ) const
{
  ...
  usr::fout( fmt, mtitle, mkstr( MeanPhotons() ), "Photons" );
  usr::fout( fmt, "\\text{Gain}", mkstr(  Gain() ), "mV-ns"   );
  usr::fout( fmt, "\\sigma_\\text{com}", mkstr(  CommonNoise() ), "mV-ns"   );
  usr::fout( fmt, "\\sigma_\\text{pix}", mkstr(  PixelNoise() ), "mV-ns"   );
  usr::fout( fmt,
             "P_\\text{ct}",
             mkstr(  ProbCrosstalk() * 100.0 ),
             "\\%"     );
  usr::fout( fmt,
             "P_\\text{ap}",
             mkstr( ProbAfterpulse() * 100.0 ),
             "\\%"     );
  ...
  usr::fout( fmt,
             "ENF",
             mkstr( ExcessNoiseFactor( MeanPhotons() ) ),
             ""        );
}
