...
main( int argc, char const* argv[] )
{
  ...
  /*
     ...
     std::cout << "Finished Beta Plot" << std::endl;
   ...
   +p0.binomial_prob( 2, 2 ) * p0.ap_eff( 2, 2 ) );
     ...
     std::cout << "Finished Alpha Plot" << std::endl;
     ...
     std::cout << "Finished gieger plot" << std::endl;
   ...
   */
  {
    ...
    for( unsigned i = 0; i < np; ++i ){
      const double xval = xmin + sep * i;
      x = xval;

      const double fval  = p0.ap_eff( 1,1 );
      const double cfval = i == 0 ? 0 : cf.GetY()[i-1] + fval * sep;
      const double yval  = p0.erf_ap_eff( xval, 1,1 ) * 0.99;

      g.SetPoint( i, xval, yval );
      f.SetPoint( i, xval, fval );
      cf.SetPoint( i, xval, cfval );
    }
    ...
    std::cout << "Finished CDF plot" << std::endl;
    c.SaveAsPDF( output_dir/"compare_cdf.pdf" );
  }
