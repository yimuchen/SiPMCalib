...
main( int argc, char* argv[] )
{
  ...
  if( args.CheckArg( "saveestpeak" ) ){
    mgr->PlotPeakFind(
      args.MakePDFFile( args.Arg<std::string>( "saveestpeak" ) ) );
  }
  if( args.CheckArg( "saveestgain" ) ){
    mgr->PlotGainFit(
      args.MakePDFFile( args.Arg<std::string>( "saveestgain" ) ) );
  }
  if( args.CheckArg( "saveestwidth" ) ){
    mgr->PlotWidthFit(
      args.MakePDFFile( args.Arg<std::string>( "saveestwidth" ) ) );
  }
  if( args.CheckArg( "saveestpoisson" ) ){
    mgr->PlotPoissonFit(
      args.MakePDFFile( args.Arg<std::string>( "saveestpoisson" ) ) );
  }

  const bool runfit = args.CheckArg( "savefit" ) ||
